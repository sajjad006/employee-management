<?php
	...
	if (empty($_SESSION['uname']) || empty($_SESSION['pass']) || empty($_SESSION['utype']) || $_SESSION['utype'] !== 'Admin') {
		...
		if (isset($_SESSION['error'])) {
			...
			elseif ($error=='csv') {
				?><script type="text/javascript">alert('Please select a file with .csv extension.')</script><?php
			}
...
<html>
<head>
	<title>ADD EMPLOYEE | CREATIVE TECHNOTEX PVT. LTD.</title>
	<style type="text/css">
		label{
			display: block;
			width: 300px;
		...
		button{
			background-color: #ca67f9;width: 200px;padding: 10px;height: 50px;color: white;
		}
...
<body>
	...
	<center>
		...
		<button onclick="document.getElementById('half').style.display='none';document.getElementById('full').style.display='block';document.getElementById('csv').style.display='none';">PROVIDE EMPLOYEE DETAILS</button>

		<button onclick="document.getElementById('half').style.display='block';document.getElementById('full').style.display='none';document.getElementById('csv').style.display='none';">ASK EMPLOYEE TO UPLOAD DETAILS</button>
		<button onclick="document.getElementById('half').style.display='none';document.getElementById('full').style.display='none';document.getElementById('csv').style.display='block';">UPLOAD CSV FILE</button>
	...
	<div style="display: none;" id="csv">
		<center>	
			<form method="POST" enctype="multipart/form-data" action="includes/csv.php">
			
			<label><h3>Select CSV File:</h3></label>
			<input type="file" name="csv" required>
			<br>

			<label><h3>Select Employee Image (in order)</h3></label>
	...
	<div style="display: none;" id="half">
		<center>
			<form method="POST" action="includes/addEmployee_process.php">
				<input type="text" name="first_name" placeholder="Employee first name" required><br>
				<input type="text" name="last_name" placeholder="Employee last name" required><br>
				<input type="text" name="mobile" placeholder="Employee Mobile" required><br>
				<input type="submit" name="halfsubmit" value="ADD EMPLOYEE"><br>
			</form>
		</center>
	</div>

	<div style="display: none;" id="full">
		<form method="POST" action="includes/addEmployee_process.php" enctype="multipart/form-data">
			...
			<fieldset>
				...
				<input type="text" name="emp_first_name" required>
				<br>
	...
	<?php
		for ($i=1; $i < 3; $i++) { 
			...
			<center><input type="submit" name="submit" value="ADD EMPLOYEE" style="display: block;float: none;"></center>
